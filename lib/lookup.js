...
const enterSeries = async (showName, list) => {
...
const discoverShow = async (program, showName) => {
  let body
  try {
    program.log.debug('lookup: [external call] discover %s', showName)
    const payload = {
      url:
  ...
  const list = results.map(({ show: { name, premiered, url } }, i) => {
    return {
...
const getShow = async (program, showName, showId) => {
  let body
  try {
    program.log.debug('lookup: [external call] get %s (%s)', showId, showName)
    const payload = {
...
module.exports = async (program, cache, showName, options) => {
  ...
  const episodes = remoteShow._embedded.episodes.map(
    ({ season, number, airstamp }) => {
