...
var getPossibleShows = function(program, showName, done) {
  ...
  request(payload, function (e, response, body) {
    ...
    enterSeries(results.length, function (e, option) {
      if (e) return done(e);
...
var getShow = function(program, showName, showId, done) {
  ...
  request(payload, function (e, response, body) {
    if (e) {
      program.log.error("no show found for id %s (%s), response error", showId, showName, e);
      return done(e);
    }
    ...
    try {
      ...
      program.log.error("no show found for id %s (%s), parsing error", showId, showName, e);
      return done(e);
    }
  });
}
...
module.exports = function(program, showName, options, done) {
  async.waterfall([
    function (next) {
      ...
      if (showId) return next(null, showId);

      program.log.warn("no serieId found for '%s'", showName);
      if (!options.discover) return next("unknown serie");
    ...
    function (showId, next) {
      ...
      if (!options.ignoreCache && moment(show.updated).add('1 day').isBefore(moment())) return next(null, show.episodes);
      if (!options.ignoreCache && show.info.status === 'Ended') return next(null, show.episodes);

      getShow(program, showName, showId, function (e, show) {
        if (e) return next(e);

        program.config.set('series:' + showName + ':updated', moment());
        program.config.set('series:' + showName + ':info:status', show.status);

        program.log.debug("found %s episodes for %s (%s)", show._embedded.episodes.length, showId, showName);
